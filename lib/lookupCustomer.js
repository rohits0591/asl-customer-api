...
/**
 * Fetches the TPIN record for a number. Looks up by document ID first (the
 * expected shape - doc id === normalized ANI), then falls back to a query on
 * the Customer_ANI field. The fallback exists because manually adding/editing
 * a document in the Firebase console can leave it with an auto-generated or
 * mistyped document ID even when the Customer_ANI field itself is correct.
 */
async function lookupTpinByAni(rawMobile) {
  ...
  const snap = await db.collection('tpins').where('Customer_ANI', '==', ani).limit(1).get();
...
/**
 * Returns every client record whose number (checking all five contact fields -
 * ENT_MOBILE_NO, ENT_MOBILE_NO_2, AR_MOBILE_NUMBER, C1 Number, C2 Number)
 * matches, AND whose DOB matches. Used by the DOB + registered-number lookup endpoint.
 */
